...
export default function ListProjectsByCampaignPage() {
  ...
  const order_dir = (getString('order_dir', 'asc') as SortDir) ?? 'asc';
  ...
  const { fundData, campaignsData, projectsData, isLoading, error, totalProjects, total_pages, mutate } = useCampaignProjects({
    campaignId,
    page,
    limit,
    order_by: sortField,
  ...
  const handleClearFilters = () => {
    ...
    updateQuery({ search: '', order_by: 'id', order_dir: 'asc', funding_status: 'all' });
  };
  ...
  if (isLoading || ((!fundData || !campaignsData) && !error)) {
    return (
      <>
    ...
    <>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="subtitle1" color="text.secondary">
          Showing <strong>{projectsData?.length || 0}</strong> of <strong>{totalProjects}</strong> projects
          {search && (
            <span>
              {' '}
          ...
          <Stack spacing={GRID_COMMON_SPACING}>
            {projectsData && projectsData.length > 0 ? (
              projectsData.map((project) => (
                <ListProjectsCard key={project.id} project={project} fund={fundData} campaign={campaignsData} />
              ))
            ) : (
              <Box textAlign="center" p={4} borderRadius={2} bgcolor="background.paper" boxShadow={1}>
                ...
                <Typography variant="body2" color="text.secondary" mb={2}>
                  {search || funding_status !== 'all'
                    ? `No results found with current filters`
                    : 'There are currently no projects in this campaign'}
                </Typography>
                {(search || funding_status !== 'all') && (
